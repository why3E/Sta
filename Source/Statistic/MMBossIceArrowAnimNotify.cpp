...
#include "MMBossIceArrowAnimNotify.h"
#include "ImpactPointInterface.h"
#include "MidBossEnemyCharacter.h"
#include "MyIceArrow.h"
#include "SESSION.h"

void UMMBossIceArrowAnimNotify::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation, const FAnimNotifyEventReference& EventReference)
{
    ...
    if (ImpactPointOwner && BossCharacter && BossCharacter->GetIceArrowClass())
    {
        ...
        if (IceArrow)
        {
            ...
            IceArrow->Fire(TargetLocation); // 방향 계산은 내부에서 자동 처리
        }
    }
}